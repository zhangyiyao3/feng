...
from factors.stock import get_finance_factors, get_tec_factors
...
def is_buy(market, symbol, bars: List[RawBar]) -> {}:
    ...
    if c.signals['倒1形态'] in [Signals.LI0.value]:
        ...
        buy_flg = True
    if buy_flg:
        factors.update(get_finance_factors(market, symbol, c))
        factors.update(get_tec_factors(c))
...
def write_excel(df):
    ...
    if os.path.exists(addr) and os.path.isfile(addr):
        ...
        title = [
            ...
            '总cash/亿', 'ma由高至低(含股价)', '向上ma', '向下ma', '信号至今涨跌%', '信号至今单Bar涨跌%',
...
def select():
    ...
    for i in range(0, len(list)):
        try:
            ...
            traceback.print_exc()
            print("{} 执行失败 - {}".format(symbol, e))
    # df_factors.round(2)  #round此处不知为何失效
    print(df_factors)
